const ChatView = ({
  ...
  const [inputHeight, setInputHeight] = useState(43); // Initial input height
  const [replyBarHeight, setReplyBarHeight] = useState(0); // Height of reply bar
  ...
  // Track input container height changes and keep scroll at bottom
  useEffect(() => {
    if (inputContainerRef.current) {
      const resizeObserver = new ResizeObserver((entries) => {
        for (let entry of entries) {
          ...
          // Keep scroll at bottom when input grows
          if (contentRef.current) {
            const { scrollTop, scrollHeight, clientHeight } = contentRef.current;
            const isNearBottom = scrollHeight - scrollTop - clientHeight < 100;
            if (isNearBottom) {
  ...
  // Track reply bar height and scroll to bottom
  useEffect(() => {
    setReplyBarHeight(replyingTo ? 58 : 0);
    
    // Scroll to bottom when reply bar appears
    if (replyingTo && contentRef.current) {
      const { scrollTop, scrollHeight, clientHeight } = contentRef.current;
      const isNearBottom = scrollHeight - scrollTop - clientHeight < 100;
      if (isNearBottom) {
  ...
  // Calculate total input area height (input + reply bar if visible)
  const totalInputAreaHeight = inputHeight + replyBarHeight;
  ...
  return (
    <div style={{
      ...
      <div
        ...
        style={{
          padding: '70px 18px',
          paddingBottom: showEmojiPicker ? '370px' : `${totalInputAreaHeight + 18}px`,
          flex: 1,
          overflowY: 'auto',
      ...
      {showScrollButton && (
        <button
          ...
          style={{
            position: 'fixed',
            right: '16px',
            bottom: showEmojiPicker ? '375px' : `${totalInputAreaHeight + 18}px`,
            width: '40px',
            height: '40px',
            borderRadius: '50%',
            background: '#fff',
