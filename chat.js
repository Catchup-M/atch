...
const ChatView = ({
  ...
  // Handle reply click to highlight original message
  const handleReplyClick = (replyToText, replyToImageUrl) => {
    // Find the original message index
    let originalIndex = -1;
    
    if (replyToImageUrl) {
      // If replying to an image, find by imageUrl
      originalIndex = messages.findIndex(m => m.type === 'image' && m.imageUrl === replyToImageUrl);
    } else {
      // If replying to text, find by text
      originalIndex = messages.findIndex(m => m.text === replyToText);
    }
    ...
    if (originalIndex !== -1) {
      ...
      // Set highlight
      setHighlightedMessageIndex(originalIndex);
      
      // Scroll to the message
      setTimeout(() => {
        const messageElements = contentRef.current?.querySelectorAll('.message-bubble');
        if (messageElements && messageElements[originalIndex]) {
          messageElements[originalIndex].scrollIntoView({ behavior: 'smooth', block: 'center' });
        }
      }, 50);
  ...
  return (
    <div style={{
      ...
      zIndex: 2,
      background: '#f5f5f5',
      backgroundImage: 'url(https://i.ibb.co/HfYZMyTL/Screenshot-20251221-162024.jpg)',
