...
const ImageViewer = ({ images, initialIndex = 0, onClose, sender = "Unknown", allMessages = [] }) => {
  ...
  const [verticalSwipe, setVerticalSwipe] = useState(0);
  ...
  const handleTouchStart = (e) => {
    ...
    if (e.touches.length === 2) {
      ...
      if (imageZoom > 1) {
        ...
        // Don't set isDragging here - wait to determine direction
      }
      setIsPinching(false);
    }
  };
  ...
  const handleTouchMove = (e) => {
    ...
    if (e.touches.length === 2 && isPinching) {
      ...
      if (imageZoom > 1 && !isPinching) {
        ...
        const panDeltaX = touch.clientX - lastTouchX.current;
        const panDeltaY = touch.clientY - lastTouchY.current;
        
        let newTranslateX = imageTranslateX + panDeltaX;
        let newTranslateY = imageTranslateY + panDeltaY;
        ...
        if (!isVerticalSwiping && !isDragging) {
        ...
        if (isVerticalSwiping) {
          // Continue vertical swipe for dismissal
          if (deltaY > 0) {
            setVerticalSwipe(deltaY);
            e.preventDefault();
          }
        } else if (isDragging) {
  ...
  return (
    <div style={{
      ...
      zIndex: 1000,
      transform: `translateY(${verticalSwipe}px)`,
      transition: isVerticalSwiping ? 'none' : 'transform 0.3s ease-out',
      opacity: isVerticalSwiping ? Math.max(0.3, 1 - verticalSwipe / 400) : 1
    }}>
      {/* Header */}
      <div style={{
        display: 'flex',
